...
export default function DetailDesktop({ slug }: { slug: string }) {
  ...
  if (loadErr && !p) return <main className="mx-auto max-w-[1200px] p-10"><p role="alert" className="rounded-[18px] border border-garis bg-kartu p-6 text-center text-sm font-bold text-[#DC2626]">{loadErr} <Link href="/" className="mt-2 block w-full rounded-[14px] bg-primer py-2.5 text-sm font-extrabold text-white">Kembali belanja</Link></p></main>;
